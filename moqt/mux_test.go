...
func TestMux_Announce_WithNilHandler_ClosesTrack(t *testing.T) {
	...
	var cancelWriteErr1 *transport.StreamError
	require.ErrorAs(t, context.Cause(mockStream.Context()), &cancelWriteErr1)
	assert.Equal(t, transport.StreamErrorCode(SubscribeErrorCodeNotFound), cancelWriteErr1.ErrorCode)
	_, readErr1 := mockStream.Read(make([]byte, 1))
	var cancelReadErr1 *transport.StreamError
	require.ErrorAs(t, readErr1, &cancelReadErr1)
	assert.Equal(t, transport.StreamErrorCode(SubscribeErrorCodeNotFound), cancelReadErr1.ErrorCode)
}
...
func TestMux_ServeTrack_NotFound_ClosesWithError(t *testing.T) {
	...
	assert.Equal(t, transport.StreamErrorCode(SubscribeErrorCodeNotFound), cancelWriteErr.ErrorCode)
	_, readErr := mockStream.Read(make([]byte, 1))
	var cancelReadErr *transport.StreamError
	require.ErrorAs(t, readErr, &cancelReadErr)
	assert.Equal(t, transport.StreamErrorCode(SubscribeErrorCodeNotFound), cancelReadErr.ErrorCode)
}
...
func TestMux_ServeAnnouncements_InitSendsExistingAnnouncements(t *testing.T) {
	...
	for name, tc := range tests {
		synctest.Test(t, func(t *testing.T) {
			...
			aw := newAnnouncementWriter(mockStream, tc.writerPrefix, 0, 0, nil, nil)
...
func TestMux_ServeAnnouncements_AncestorAndDescendantReceive_AnnounceBefore(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		rootAW := newAnnouncementWriter(rootStream, "/", 0, 0, nil, nil)
		...
		shareAW := newAnnouncementWriter(shareStream, "/share/", 0, 0, nil, nil)
...
func TestMux_ServeAnnouncements_InvalidPrefix_ClosesWithError(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		aw := newAnnouncementWriter(mockStream, "/test/", 0, 0, nil, nil)
		...
		for time.Now().Before(deadline) {
			...
			found = errors.As(context.Cause(mockStream.Context()), &streamErr)
			if found {
				break
			}
			time.Sleep(10 * time.Millisecond)
		...
		var cancelWriteErr *transport.StreamError
		require.ErrorAs(t, context.Cause(mockStream.Context()), &cancelWriteErr)
		assert.Equal(t, transport.StreamErrorCode(AnnounceErrorCodeInvalidPrefix), cancelWriteErr.ErrorCode)
		_, readErr := mockStream.Read(make([]byte, 1))
		var cancelReadErr *transport.StreamError
		require.ErrorAs(t, readErr, &cancelReadErr)
		assert.Equal(t, transport.StreamErrorCode(AnnounceErrorCodeInvalidPrefix), cancelReadErr.ErrorCode)
...
func TestMux_ServeAnnouncements_InitWriteError_ClosesWithInternalError(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		var cancelWriteErr *transport.StreamError
		require.ErrorAs(t, context.Cause(mockStream.Context()), &cancelWriteErr)
		assert.Equal(t, transport.StreamErrorCode(AnnounceErrorCodeInternal), cancelWriteErr.ErrorCode)
		_, readErr := mockStream.Read(make([]byte, 1))
		var cancelReadErr *transport.StreamError
		require.ErrorAs(t, readErr, &cancelReadErr)
		assert.Equal(t, transport.StreamErrorCode(AnnounceErrorCodeInternal), cancelReadErr.ErrorCode)
...
func TestMux_ServeAnnouncements_ContextCancel_StopsLoop(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		var streamErr *transport.StreamError
		if errors.As(context.Cause(mockStream.Context()), &streamErr) {
			t.Fatal("unexpected CancelWrite")
		}
		_, readErr := mockStream.Read(make([]byte, 1))
		if errors.As(readErr, &streamErr) {
			t.Fatal("unexpected CancelRead")
		}
	})
}
...
func TestMux_ServeAnnouncements_SlowSubscriber_NoDeadlock(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		aw := newAnnouncementWriter(mockStream, "/slow/", 0, 0, nil, nil)
...
func TestMux_ServeAnnouncements_MultipleListeners_ReceiveAnnouncement(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		aw1 := newAnnouncementWriter(mock1, "/multi/", 0, 0, nil, nil)
		...
		aw2 := newAnnouncementWriter(mock2, "/multi/", 0, 0, nil, nil)
...
func TestMux_Announce_ClosesBusySubscriber(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		aw := newAnnouncementWriter(mockStream, "/busy/", 0, 0, nil, nil)
...
func TestMux_Publish_InitSendsExistingAnnouncements(t *testing.T) {
	...
	aw := newAnnouncementWriter(mockStream, "/pubinit/", 0, 0, nil, nil)
...
func TestMux_Publish_AfterServeAnnouncements_SendsAnnouncement(t *testing.T) {
	...
	aw := newAnnouncementWriter(mockStream, "/pubafter/", 0, 0, nil, nil)
...
func TestMux_PublishFunc_InitSendsExistingAnnouncements(t *testing.T) {
	...
	aw := newAnnouncementWriter(mockStream, "/pubfuncinit/", 0, 0, nil, nil)
...
func TestMux_PublishFunc_AfterServeAnnouncements_SendsAnnouncement(t *testing.T) {
	...
	aw := newAnnouncementWriter(mockStream, "/pubfuncafter/", 0, 0, nil, nil)
...
func TestMux_ServeAnnouncements_ConcurrentAnnounce_NoDeadlock(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		for range listeners {
			...
			aw := newAnnouncementWriter(ms, "/race/", 0, 0, nil, nil)
			aws = append(aws, aw)
...
func TestMux_ServeAnnouncements_AncestorAndDescendantReceive(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Wait for both streams to receive a write from Announce
		select {
		case <-rootWriteCh:
		case <-time.After(500 * time.Millisecond):
			t.Fatal("timeout waiting for Write on root stream")
		}
		select {
		case <-shareWriteCh:
		case <-time.After(500 * time.Millisecond):
			t.Fatal("timeout waiting for Write on share stream")
		}
