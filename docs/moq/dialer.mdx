...
<Warning>
**Deprecated: `DialWebTransport` and `DialQUIC`**

The transport-specific methods are deprecated and will be removed in a future
release. They take a host and a path that `Dial` derives from the URL, so they
carry no information the URL does not. Migrate to `Dial`:

```go
	// WebTransport
	-sess, err := dialer.DialWebTransport(ctx, "host:port", "/path", mux)
	+sess, err := dialer.Dial(ctx, "https://host:port/path", mux)

	// Native QUIC
	-sess, err := dialer.DialQUIC(ctx, "host:port", "/path", mux)
	+sess, err := dialer.Dial(ctx, "moqt://host:port/path", mux)
```

The URL form works for an already-resolved address too, including an IPv6
literal (`moqt://[::1]:4433/live`).
</Warning>
