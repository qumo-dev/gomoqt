...
`msf.Broadcast` does not implement `TrackInfoProvider` itself, so tracks registered through it report the default zero-valued properties unless the handler you pass to `RegisterTrack` implements `TrackInfoProvider` directly.
