...
When `AcceptAnnounce` is called, the `TrackMux`'s hop ID is automatically sent as `ExcludeHop` in the ANNOUNCE_REQUEST message. This prevents announcement loops in relay topologies. See [Relay — Hop ID and Loop Avoidance](/moq/relay#hop-id-and-loop-avoidance) for details.
</Note>
