...
- **moq-web/internal/message:** Message `encode` now serializes the whole body into a single buffer (new sync `MessageEncoder`) and issues **one** `Writer.write`, instead of one allocation + one awaited write per field. Measured via `message_benchmark.ts`: `GroupMessage.encode` −63%, `SubscribeMessage.encode` −74% (the encode path was ~2× its `decode`; it is now faster). `MessageEncoder` also UTF-8-encodes strings exactly once for correct sizing, where the removed `len`/`stringLen` pre-sizing counted UTF-16 units. Wire format and the `encode → Error | undefined` contract are unchanged — the same varint/string primitives in the same field order, verified by the decode round-trip tests; the per-field write-error unit tests were consolidated to the single combined write. The standalone `writeVarint`/`writeString` helpers are unchanged (still used for stream-type prefixes), and the `decode` path was already optimal (one read + sync `parse*`).
