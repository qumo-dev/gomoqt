...
- **moq-web:** `Session.closed`, a `Promise<void>` that resolves when the session terminates — via `close()`, `closeWithError()`, a peer-initiated close, or the underlying transport dropping. It never rejects, and mirrors Go's `Session.Context().Done()` and WebTransport's `closed`, giving consumers a single primitive to await for reconnect/cleanup (previously the only public lifecycle signal was `ready`).
