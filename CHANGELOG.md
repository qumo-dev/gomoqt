...
- **moqt:** New system-level fan-out benchmarks — `BenchmarkFanOut_SteadyState` (1 unpaced publisher → N long-lived subscribers over real loopback QUIC; subs-count sweep at 16K) and `BenchmarkFanOut_Latency` (per-subscriber one-way publish→deliver p50/p95/p99/max + fairness spread). These fill the gap no existing harness covers: steady-state, long-lived multi-subscriber fan-out — `BenchmarkBroadcastServer_HighLoad` reconnects every client every iteration and so measures connection churn, not fan-out. Early numbers confirm the O(N)-encode architecture (`Broadcast.ServeTrack` runs the published handler once per subscriber): per-subscriber throughput collapses ~4× from 1→4 subscribers as the core saturates. Real-QUIC integration benches, run in the `benchmark-integration` job, not on PRs.
