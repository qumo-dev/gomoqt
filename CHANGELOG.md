...
- **moqt/internal/message:** `ReadBytes`/`ReadStringArray` no longer `panic` on a maliciously large varint length/count — they return an error, and `ReadStringArray` bounds the count against the remaining buffer — preventing a remote-triggered panic/OOM DoS.
