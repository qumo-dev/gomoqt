...
- **moqt:** Updated the wire protocol and APIs from MoQ Lite draft-04 to draft-05, including SETUP negotiation, ANNOUNCE_REQUEST/ANNOUNCE_OK, TRACK metadata, SUBSCRIBE_END, timestamp deltas, and the `moq-lite-05` ALPN token. The Go and TypeScript clients now implement the draft-05 session, track, probe, and frame behavior.
...
- **docs:** Migrated the documentation site from Hugo/Hextra to **Mintlify**. All 27 English pages moved from `docs/moqt/content/en/**/*.md` to `docs/**/*.mdx`, with the Hextra shortcodes converted to Mintlify MDX components: `cards`/`card` → `<Columns>`/`<Card>`, `tabs`/`tab` → `<Tabs>`/`<Tab>`, `details` → `<Accordion>`, `steps` (heading-delimited) → `<Steps>`/`<Step>`, and `> [!NOTE]`/`> [!TIP]` callouts → `<Note>`/`<Tip>`. Section landing pages (`_index.md`) become explicit `overview.mdx` pages, so sidebar order now lives in `docs.json` rather than per-page `weight:` front matter — the duplicate-`weight` ambiguity fixed earlier this cycle is structurally gone. Hugo's directory-relative links (`../errors/#built-in-error-codes`) are rewritten as absolute page paths (`/moq/errors#built-in-error-codes`), and the Hextra home layout (`hextra/hero-*`, `feature-grid`) is replaced by a plain MDX landing page, since Mintlify has no hero shortcode. Verified with `mint broken-links`: 0 parse errors, 0 broken links. The site moves from `https://qumo-dev.github.io/gomoqt/` to `https://gomoqt.mintlify.site/`; the two in-repo references (`.github/ISSUE_TEMPLATE/config.yml`, `moqt/README.md`) follow.
...
- **docs:** Pointed the MoQ Lite specification links in all six language READMEs at **draft-05** (18 links, 3 per file). They still pointed at `draft-lcurley-moq-lite-04.html` after the draft-04 → draft-05 protocol migration, so every README advertised a spec version the code no longer implements. The code, `SPECIFICATION.md`, and the documentation site were already on draft-05; the documentation site's own spec links are versionless datatracker URLs and were unaffected.
...
- **docs:** Removed the Hugo site (`docs/moqt/`, including its own `go.mod` that resolved the Hextra theme module, `hugo.yaml`, custom CSS, layouts, and the `i18n/` files for five languages that had no corresponding `content/` translations) and the `pages.yml` GitHub Pages workflow it fed. Mintlify deploys from the repository through its GitHub App, so no build workflow replaces it. The `**/docs/moqt/public` ignore rule is replaced by `**/.mint/`.
