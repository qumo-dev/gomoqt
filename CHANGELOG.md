...
- **moqt:** `Session.CloseWithError` now propagates cancellation to every Track and Group instead of relying on connection-close to unblock in-flight stream reads. It cancels the session's own context (so `AcceptStream`/`OpenUniStream` calls return) and cascades `CancelRead`/`CancelWrite` onto every outstanding group stream before `wg.Wait()`. quic-go and webtransport-go both guarantee `CancelRead`/`CancelWrite` unblocks an in-flight `Read`/`Write` immediately, so a goroutine parked in `GroupReader.ReadFrame` (e.g. a consumer's `for frame := range gr.Frames(buf)`) or a publisher parked in a group `Write` exits deterministically — independent of whether the transport propagates the close into the blocked call. This fixes a deadlock where `CloseWithError`'s `wg.Wait` hung because a `webtransport-go` receive-stream goroutine stalled and never woke the blocked read (qumo #205, `TestRTMPInterop_Matrix` ~50% hang on Windows). Two compounding bugs were fixed along the way: `Session.CloseWithError` previously touched no Track at all, and `TrackReader.dequeued` — the field meant to track accepted groups for cancellation — was never populated, so an already-accepted `GroupReader` was tracked nowhere; cancel now goes through the already-maintained `groupReaderManager`/`groupWriterManager` active-group sets.
