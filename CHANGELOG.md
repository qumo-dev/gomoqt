...
- **moq-web/internal/message:** Message `encode` now serializes the whole body into a single buffer (new sync `MessageEncoder`) and issues **one** `Writer.write`, instead of one allocation + one awaited write per field. Measured via `message_benchmark.ts`: `GroupMessage.encode` −63%, `SubscribeMessage.encode` −74% (the encode path was ~2× its `decode`; it is now faster). `MessageEncoder` also UTF-8-encodes strings exactly once for correct sizing, where the removed `len`/`stringLen` pre-sizing counted UTF-16 units. Wire format and the `encode → Error | undefined` contract are unchanged (decode round-trips and Go interop tests pass); the per-field write-error unit tests were consolidated to the single combined write. The standalone `writeVarint`/`writeString` helpers are unchanged (still used for stream-type prefixes), and the `decode` path was already optimal (one read + sync `parse*`).
- **CI:** `go-required-stub.yml`, a companion to `go.yml` that reports the required `build`/`race` status checks as passing on PRs that touch no Go files. `go.yml` is path-filtered to Go sources, so a TS-/docs-only PR never triggered it and stayed permanently blocked on those required checks (GitHub does not auto-pass a path-skipped required check). The stub runs on the complement of `go.yml`'s `paths` (via `paths-ignore`), so Go PRs are still gated by the real jobs.
