...
- **moqt:** `TrackWriter.openGroupWithSequence` now coalesces the uni-stream type byte and the GROUP message into a **single `Write`** on the new group's stream (previously two separate `Encode`→`Write` calls). Wire output is byte-identical, guarded by `TestGroupMessage_AppendEncode_WireIdentical` (spans all varint length boundaries for both fields). Added `GroupMessage.AppendEncode` — an append-style sibling of `Encode` sharing its exact wire bytes — and the `MaxVarintLen` constant. Motivation: halve the per-group-open `Write` interactions with the QUIC `SendStream` (one stream-lock / flow-control / sendQueue copy instead of two), relevant to high-fan-out relaying where every subscriber opens a stream per group. Caveat (measured): on the in-memory-stream microbenchmark (`BenchmarkTrackWriter_OpenGroup`) this is allocation-neutral-to-slightly-negative — allocs/op 7→6 but B/op 328→352 (+24), because the small stack buffer escapes through the `io.Writer` interface `Write`; the intended benefit is on real QUIC streams and is not exercised by the `-short`-gated CI suite.
