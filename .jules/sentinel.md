...
## YYYY-MM-DD - Fix Remote DoS in Message Parsing
**Vulnerability:** panic() used to handle oversized bounds when parsing untrusted input.
**Learning:** Panics in network packet parsing functions allow attackers to crash the server with maliciously crafted payload sizes.
**Prevention:** Always return proper error values for validation failures on untrusted input instead of panicking.
