...
## 2024-05-24 - DoS DoS via unconstrained math.MaxInt payload checks
**Vulnerability:** A denial-of-service (DoS) vulnerability caused by unconstrained lengths triggering panics on remote inputs. `ReadBytes` and `ReadStringArray` panicked when string array size exceeded `math.MaxInt` instead of returning errors.
**Learning:** Panicking on malformed or oversized untrusted network input introduces a DoS risk.
**Prevention:** Always return proper errors (e.g., `ErrMessageTooLarge`) and gracefully disconnect instead of panicking on remotely-supplied sizes. Use `MaxMessageSize` limits instead of platform-dependent bounds.
