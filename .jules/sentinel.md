...
## 2024-07-19 - Fix DoS via panic on large varints
**Vulnerability:** A Denial of Service (DoS) vulnerability caused by `panic()` calls when untrusted network input contained varint lengths exceeding `math.MaxInt`.
**Learning:** Using `panic()` to handle malformed or oversized untrusted network input (e.g., lengths exceeding `math.MaxInt`) introduces a remote DoS vulnerability. The application will crash.
