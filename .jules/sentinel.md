...
## YYYY-MM-DD - [DoS via Panic in Message Parsing]
**Vulnerability:** Untrusted network input parsing functions (`ReadBytes`, `ReadStringArray`) trigger a `panic` if the encoded size exceeds `math.MaxInt`.
**Learning:** Using `panic()` to handle malformed or maliciously large inputs in network parsing routines introduces a remote Denial of Service (DoS) vulnerability, allowing an attacker to crash the server with a single malformed message.
**Prevention:** Always fail securely by returning standard errors (e.g., `errors.New("byte slice too large")`) instead of panicking on untrusted input bounds validation.
