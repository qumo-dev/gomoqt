...
## YYYY-MM-DD - [DoS via panic on large varint sizes]
**Vulnerability:** Untrusted network data parsing used `panic()` when checking length limits against large values.
**Learning:** Using `panic()` in network paths exposes the application to remote Denial of Service (DoS) attacks.
**Prevention:** Always fail securely by returning appropriate errors (e.g., `errors.New("byte slice too large")`) for untrusted input bounds validation.
