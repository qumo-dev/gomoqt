...
## YYYY-MM-DD - Fix DoS via untrusted varint length parsing panic
**Vulnerability:** A remote DoS vulnerability where parsing an oversized byte slice length or string array count triggers a panic.
**Learning:** Using `panic()` for validating untrusted network payloads is unsafe and can be trivially exploited to crash the server.
**Prevention:** Always validate untrusted bounds and return descriptive errors (e.g., `ErrMessageTooLarge`) instead of panicking.
